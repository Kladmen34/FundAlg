...
void printPersons(List *list) {
    ...
    do {
        ...
        printf("\n------------------------------\n%s\n%s\n%s\n%s\n%d\n%f\n------------------------------\n",
               ...
               person->birthDate, person->gender, person->salary);
...
short isCorrectGender(char *string) {
...
short isCorrectDate(const char *string) {
    int day, month, year;
    int dotCount = 0;
    int x = 0;
    while (string[x] != '\0') {
...
short readPerson(FILE *file, Person **result) {
    short isLastPerson;
    char *lastname, *name, *patronymic, *date, *genderString, *salaryString;
    double salary;
    Gender gender;
    short statusCode;
    ...
    statusCode = readString(file, &genderString, "\t \n", isLetter);
    if (statusCode != 0) {
        freeAll(4, lastname, name, patronymic, date);
        return statusCode;
    }
    switch (isCorrectGender(genderString)) {
        case 0:
            freeAll(5, lastname, name, patronymic, date, genderString);
            return 5;
        case 1:
            gender = male;
            break;
    ...
    if (!isCorrectDouble(salaryString, &salary)) {
        freeAll(6, lastname, name, patronymic, date, genderString, salaryString);
        return 6;
    }
    ...
    *result = person;
    freeAll(2, salaryString, genderString);
    if (isLastPerson)
        return -1;
    return 0;
}
...
short saveToFile(char *filename, List *list) {
    ...
    do {
        ...
        if (fprintf(file, "%s\t%s\t%s\t%s\t%s\t%f\n", person->lastname,
                    ...
                    person->gender == male ? "male" : "female", person->salary) < 0) {
            fclose(file);
            return 3;
        }
    } while (listHasNextNode(list, node));
...
int searchPerson(List *list, char *lastname, char *name, char *patronymic,
                 char *birthDate, Gender *gender, double *salary) {
    Node *node = NULL;
    int x = -1;
    do {
        x++;
...
int main(int argc, char *argv[]) {
    ...
    printPersons(list);
    printf("\nINDEX: %d\n", searchPerson(list, "Nikulin", NULL, NULL, NULL, NULL, NULL));
